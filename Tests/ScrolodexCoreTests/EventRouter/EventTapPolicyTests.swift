...
struct EventTapPolicyTests {
	...
	func doesNotHandleKeyUp() {
		...
		#expect(
			EventTapPolicy.action(for: .keyUp, triggerHeld: true, permissionsAvailable: true)
				== .passThrough)
	}
	...
	func triggerPointerActions() {
		#expect(
			EventTapPolicy.action(for: .scrollWheel, triggerHeld: true, permissionsAvailable: true)
				== .handleScroll)
		#expect(
			EventTapPolicy.action(for: .leftMouseDown, triggerHeld: true, permissionsAvailable: true)
				== .handleClickConfirm)
		#expect(
			EventTapPolicy.action(for: .scrollWheel, triggerHeld: false, permissionsAvailable: true)
				== .passThrough)
	}
	...
	func passesThroughWhenPermissionsUnavailable() {
		#expect(
			EventTapPolicy.action(for: .flagsChanged, triggerHeld: true, permissionsAvailable: false)
				== .passThrough)
		#expect(
			EventTapPolicy.action(for: .scrollWheel, triggerHeld: true, permissionsAvailable: false)
				== .passThrough)
		#expect(
			EventTapPolicy.action(for: .leftMouseDown, triggerHeld: true, permissionsAvailable: false)
				== .passThrough)
	}
	...
	func doesNotReenableDisabledTapWithoutPermissions() {
		#expect(
			EventTapPolicy.action(
				for: .tapDisabledByUserInput, triggerHeld: false, permissionsAvailable: false)
				== .passThrough)
		#expect(
			EventTapPolicy.action(
				for: .tapDisabledByTimeout, triggerHeld: false, permissionsAvailable: false)
				== .passThrough)
	}

	@Test("identifies disabled tap events")
	func identifiesDisabledTapEvents() {
		#expect(EventTapPolicy.isDisabledEvent(.tapDisabledByUserInput))
		#expect(EventTapPolicy.isDisabledEvent(.tapDisabledByTimeout))
		#expect(EventTapPolicy.isDisabledEvent(.scrollWheel) == false)
	}
	...
	func keyDownScopedBindings() {
		...
		let triggerBBinding = KeyboardHotkeyConfiguration(
			flags: [.maskCommand, .maskAlternate, .maskControl], keyCode: 49)
